...
class ColourFinder:
    def __init__(self):
        self.colours = [
            Colour('r', 177.5, 5.65),  # red object. Special case -> (larger than 177.5) or (between 0 and 5)
            Colour('o', 5.65, 22.76),  # orange object
    ...
    def __find_hsb_colour_name(self):
        ...
        if brightness >= self.white_min_brightness and saturation <= self.white_max_saturation:
            return 'w'  # white
        elif (hue >= self.colours[0].minimum_hue) or (hue <= self.colours[0].maximum_hue):  # special case for red
            return 'r'  # red
        else:
            # check for orange, yellow, green and blue
            for colour_instance in self.colours[1:]:
                if (hue > colour_instance.minimum_hue) and (
                        hue <= colour_instance.maximum_hue):  # the hue is within the colour instance's hue range
